...
const hotStack: DeepPartial<FullConfig> = {
  monerod: { moneroNodeDomain: "node.mydomain.com" },
  stagenet: {
    isStagenetNode: true,
  ...
  services: {
    ...
    isWatchtower: true,
    isMonitoring: true,
    grafanaDomain: "grafana.mydomain.com",
    isTraefik: true,
...
const makeConfig = (overrides: DeepPartial<FullConfig> = {}): FullConfig =>
  makeFullConfig(hotStack, overrides);
...
describe("monerod <-> p2pool connection", () => {
  ...
  it("propagates a custom ZMQ pub port from monerod to p2pool, and never publishes it on the host", () => {
    ...
    const services = generateAllServices(
      makeConfig({ monerod: { zmqPubEnabled: true, zmqPubBindPort: customPort } })
    );
    const monerod = services.monerod.code.monerod as ContainerSpec;
